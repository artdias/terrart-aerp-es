...
export async function assignEquipment(formData: FormData) {
  try {
    ...
    if (!employeeId || !productIds || productIds.length === 0) {
      return { success: false, error: "Funcionário e ao menos um produto são obrigatórios." };
    }
    ...
    for (let i = 0; i < productIds.length; i++) {
      ...
      if (!pId) {
        return { success: false, error: `Item ${i + 1}: Selecione um produto válido.` };
      }
      if (isNaN(qty) || qty <= 0) {
        return { success: false, error: `Item ${i + 1}: Informe uma quantidade válida maior que 0.` };
      }

      const product = await prisma.product.findUnique({ where: { id: pId } });
      if (!product) {
        return { success: false, error: `Item ${i + 1}: Produto não encontrado.` };
      }
      if (product.quantity < qty) {
        return { success: false, error: `Estoque insuficiente para o produto "${product.name}". Apenas ${product.quantity} disponíveis.` };
      }
    ...
    console.error("Erro em assignEquipment:", error);
    return { success: false, error: "Ocorreu um erro interno ao tentar registrar a cautela." };
  }
}
...
export async function returnEquipment(formData: FormData) {
  try {
    ...
    if (!equipmentId || !status) {
      return { success: false, error: "ID da atribuição e status de baixa são obrigatórios." };
    }
    ...
    if (!equipment || equipment.status !== "EM USO") {
      return { success: false, error: "Esta atribuição não está ativa ou já foi devolvida." };
    }
    ...
    console.error("Erro em returnEquipment:", error);
    return { success: false, error: "Ocorreu um erro interno ao tentar processar a devolução." };
  }
}
